...
"""Cluster-wise revenue — where billed revenue is earned, by region.

One row per (cluster, currency): the sum of line-item revenue provisioned in that
cluster, its human region label, and its share of that currency's total. Recurring
(bundle) vs usage is split out so you can see a region's compute base separately
from its metered spend.

Revenue is grouped per currency and never summed across them; the share % is each
cluster's slice of its own currency's total, so the percentages add to 100 within a
currency.
"""
